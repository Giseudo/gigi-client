...
const activePort = inject('activePort')
...
const animate = () => {
  const value = activePort.value === props.port
  ...
  anime({
    targets: text.value.group.position,
    y: value ? .75 : .5,
    z: value ? 1. : .25,
    duration: 300,
    easing,
  ...
  anime({
    targets: text.value.group.scale,
    x: value ? 2. : 1.,
    y: value ? 2. : 1.,
    z: value ? 2. : 1.,
    duration: 300,
    easing,
    complete: () => isLoading.value = !value
  })
...
const onClick = () => {
  if (activePort.value !== props.port) return
